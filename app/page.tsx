...
export default function Home() {
  ...
  const photos = [
    ...
    { url: "/memories/stc8.jpeg", caption: "Grateful for you 🙏" },
  ];
  ...
  const paginate = (newDirection: number) => {
    ...
    const nextPhoto = currentPhoto + newDirection;

    // Handle wrapping around
    if (nextPhoto < 0) {
      setCurrentPhoto(photos.length - 1);
    } else if (nextPhoto >= photos.length) {
      setCurrentPhoto(0);
    } else {
  ...
  const handleInstallClick = async () => {
    ...
    try {
      ...
      if (outcome === "accepted") {
        ...
        setTimeout(() => {
          alert(
            "🎉 Birthday card installed! You can now access it from your home screen! 💕"
          );
        }, 500);
      }
      ...
      alert(
        "💕 Having trouble installing? Try using your browser's menu to add this page to your home screen!"
      );
    }
  };
  ...
  return (
    <div
      className={`h-screen w-full ${bgClass} transition-all duration-1000 relative overflow-hidden flex flex-col`}
    >
      {/* Floating Hearts */}
      ...
      <div className="sticky top-0 right-0 flex justify-end gap-2 p-4 z-[70] cursor-pointer">
        <button
          ...
          } rounded-2xl shadow-lg hover:scale-110 transition`}
          aria-label="Install app"
        >
          <Gift
            size={20}
            className={darkMode ? "text-pink-300" : "text-purple-500"}
          />
        </button>
        ...
        >
          <Moon
            size={20}
            className={darkMode ? "text-purple-300" : "text-gray-400"}
          />
        </button>
      </div>
        ...
        <button
          ...
          className={`absolute top-4 left-4 p-3 ${
            darkMode
              ? "bg-white/10 backdrop-blur-md border border-white/20"
              : "bg-white/90 backdrop-blur-md"
          } rounded-2xl shadow-lg hover:scale-110 transition z-[70] flex items-center gap-2`}
        >
          <ArrowLeft
            size={20}
      ...
      {easterEggFound && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] bg-black/40 backdrop-blur-md p-6 animate-fade-in">
          <div
            className={`${
              darkMode ? "bg-gray-900 border border-pink-500/30" : "bg-white"
            } p-8 rounded-[2.5rem] shadow-2xl max-w-sm w-full transform transition-all`}
          >
            <div className="text-center">
              ...
              >
                "Stacy, Pass my regards to your big heart and explicitly mention
                that I will never wish not to be in it. 💕"
              </p>
              <button
      ...
      {konamiUnlocked && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] bg-black/60 backdrop-blur-lg p-6 animate-fade-in">
          <div
            className={`${
              darkMode
                ? "bg-indigo-950 border border-purple-400/30"
                : "bg-white"
            } p-8 rounded-[2.5rem] shadow-2xl max-w-sm w-full relative overflow-hidden`}
          >
            ...
            <div className="text-center relative z-10">
              ...
              <div className="space-y-4">
                ...
                >
                  "You have constantly showed me what a life partner should look
                  like and you are the life partner I wish for."
                </p>
          ...
          {step === 0 && (
            <div className="text-center animate-fade-in">
              ...
              >
                ...
                miles away... the author is sorry for how things turned out in the end and wishes that you find it in you to forgive them... 
                ...
                <div
                  ...
                  className={`${
                    darkMode
                      ? "bg-white/10 backdrop-blur-md border border-white/20"
                      : "bg-white/90 backdrop-blur-md"
                  } p-6 rounded-3xl shadow-lg animate-slide-in`}
                  style={{ animationDelay: `${i * 0.3}s` }}
                >
                  <p
                    className={`text-lg ${
                      darkMode ? "text-pink-50" : "text-gray-800"
                    } text-center`}
                  >
                    {compliment}
                  </p>
            ...
            <div className="text-center animate-fade-in">
              <div className="mb-8 flex justify-center">
                <Sparkles
                  className={
                    darkMode
                      ? "text-yellow-300 animate-spin-slow"
          ...
          {step === 3 && (
            <div className="text-center animate-fade-in w-full max-w-sm mx-auto">
              <button
                ...
                className={`mb-6 flex items-center gap-2 mx-auto px-6 py-3 ${
                  darkMode
                    ? "bg-white/10 backdrop-blur-md border border-white/20 text-pink-200"
                    : "bg-white/90 backdrop-blur-md text-gray-700"
                } rounded-full shadow-lg hover:scale-105 transition active:scale-95`}
              >
                <Camera size={20} />{" "}
                {showPhotos ? "Hide Memories" : "View Memories"}
              </button>
                    ...
                    <div
                      className={`${
                        darkMode
                          ? "bg-white/10 backdrop-blur-md border border-white/20"
                          : "bg-white/90 backdrop-blur-md"
                      } p-4 rounded-[2rem] shadow-2xl mb-8 relative overflow-hidden`}
                    >
                      {/* Main Image Slider */}
                      <div className="relative aspect-square rounded-2xl overflow-hidden bg-gray-100 shadow-inner group">
                        ...
                        {/* Navigation Arrows */}
                        <button
                          className="absolute left-2 top-1/2 -translate-y-1/2 z-10 p-2 rounded-full bg-black/20 backdrop-blur-sm text-white opacity-0 group-hover:opacity-100 transition-opacity"
                          onClick={() => paginate(-1)}
                        >
                          <ChevronLeft size={24} />
                        </button>
                        <button
                          className="absolute right-2 top-1/2 -translate-y-1/2 z-10 p-2 rounded-full bg-black/20 backdrop-blur-sm text-white opacity-0 group-hover:opacity-100 transition-opacity"
                          onClick={() => paginate(1)}
                        >
              ...
              <button
                onClick={() => setStep(4)}
                className="px-8 py-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-full text-lg font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition transform active:scale-95"
              >
                Show me more! ✨
              </button>
            </div>
              ...
              <div
                className={`${
                  darkMode
                    ? "bg-white/10 backdrop-blur-md border border-white/20"
                    : "bg-white/90 backdrop-blur-md"
                } p-8 rounded-3xl shadow-2xl mb-6`}
              >
                <h1
